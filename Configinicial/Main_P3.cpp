...
int main() {
	...
	GLFWwindow *window = glfwCreateWindow(WIDTH, HEIGHT, "Practica 3 Hernandez Ramirez Miguel Angel", nullptr, nullptr);
 ...
 //     
	...
	while (!glfwWindowShouldClose(window))
	{
	   ...
	   view = glm::translate(view, glm::vec3(0.0f,0.0f,-17.0f));
		model = glm::rotate( model, 0.2f, glm::vec3( 1.0f, 0.0f, 0.0f ) ); // use to compare orthographic and perspective projection
		...
		model = glm::mat4(1);
		model = glm::translate(model, glm::vec3(7.0f, 0.0f, -17.0f));
		model = glm::rotate(model, 1.8f, glm::vec3(1.0f, 0.0f, 0.0f)); // use to compare orthographic and perspective projection
		...
		model = glm::rotate(model, 3.4f, glm::vec3(1.0f, 0.0f, 0.0f)); // use to compare orthographic and perspective projection
		model = glm::scale(model, glm::vec3(3.0f, 3.0f, 3.0f));
		glUniformMatrix4fv(modelLoc, 1, GL_FALSE, glm::value_ptr(model));
		glDrawArrays(GL_TRIANGLES, 0, 36);

		model = glm::mat4(1);
		model = glm::translate(model, glm::vec3(-2.2f, 3.0f, 0.0f));
		...
		model = glm::rotate(model, 11.2f, glm::vec3(1.0f, 0.0f, 0.0f)); // use to compare orthographic and perspective projection
		model = glm::scale(model, glm::vec3(3.0f, 3.0f, 3.0f));
		glUniformMatrix4fv(modelLoc, 1, GL_FALSE, glm::value_ptr(model));
		glDrawArrays(GL_TRIANGLES, 0, 36);
