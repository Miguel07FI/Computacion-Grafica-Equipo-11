...
int main()
{
	...
	GLFWwindow* window = glfwCreateWindow(WIDTH, HEIGHT, "Animacion basica Hernandez Ramirez Miguel Angel", nullptr, nullptr);
...
void KeyCallback(GLFWwindow* window, int key, int scancode, int action, int mode)
{
	...
	// Activar o desactivar la animación con la tecla "N"
	if (keys[GLFW_KEY_N])
	{
		animacionActivada = !animacionActivada;  // Alternar el estado de la animación
	}
}

void Animation() {
	// Verificar si la animación está activada
	if (animacionActivada)
	{
		// Cambiar la dirección de la pelota cuando alcanza los límites
		if (posBallY >= limiteSuperior) {
			moviendoArriba = false;  // La pelota se moverá hacia abajo
		}
		if (posBallY <= limiteInferior) {
			moviendoArriba = true;  // La pelota se moverá hacia arriba
		}
